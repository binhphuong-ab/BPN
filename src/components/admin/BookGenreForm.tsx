...
import { BookGenreFormData } from '@/hooks/useBookGenres';
import { generateVietnameseSlug } from '@/utils/vietnamese-slug-generating';
import { generateRandomGenreColor } from '@/models/bookgenre';
import IconSelector from './IconSelector';

interface BookGenreFormProps {
  formData: BookGenreFormData;
  onFormDataChange: (data: Partial<BookGenreFormData>) => void;
  onSubmit: (data: BookGenreFormData) => Promise<void>;
  onCancel: () => void;
  isEditing: boolean;
  isSubmitting?: boolean;
}

export default function BookGenreForm({ 
  formData, 
  onFormDataChange, 
  onSubmit, 
  onCancel, 
  ...
  const handleNameChange = (newName: string) => {
    ...
    const shouldUpdateSlug = formData.slug === '' || formData.slug === generateVietnameseSlug(formData.name);
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      <h4 className="text-lg font-semibold text-gray-900 mb-4">
        {isEditing ? 'Edit Book Genre' : 'Create New Book Genre'}
      </h4>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="Enter book genre name..."
            disabled={isSubmitting}
          />
        </div>

        <div>
          <label htmlFor="bookgenre-slug" className="block text-sm font-medium text-gray-700 mb-1">
            URL Slug *
          </label>
          ...
          <p className="text-sm text-gray-600 mt-1">
            URL-friendly version of your genre name. Will be used in URLs.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="flex space-x-2">
            ...
            <button
              type="button"
              onClick={() => onFormDataChange({ color: generateRandomGenreColor() })}
        ...
        <div className="flex items-center">
          <input
            type="checkbox"
            id="bookGenreFeatured"
            checked={formData.featured}
            onChange={(e) => onFormDataChange({ featured: e.target.checked })}
            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
            disabled={isSubmitting}
          />
          <label htmlFor="bookGenreFeatured" className="ml-2 text-sm text-gray-700">
            Featured genre (display prominently)
          </label>
        </div>
